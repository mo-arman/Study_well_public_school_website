...
export default async function StudentDashboard() {
  ...
  return (
    <>
      <PageHeader eyebrow="ERP · Student" title={`Welcome, ${name}`} description={`Class: ${classSection}`} />
      <div className="container-custom py-12">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          <DashboardCard icon={CalendarCheck} label="Attendance" value="Coming soon" />
          ...
          <DashboardCard icon={Bell} label="Notices" value="Coming soon" />
        </div>
        ...
        <p className="text-navy-ink/50 text-sm mt-10 max-w-2xl">
          ...
          pull from (spreadsheet import, accountant's software, etc.) so it
          can be connected to these cards next.
        </p>
      </div>
    </>
  );
